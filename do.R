...
for (strategy in c("GEM2")) {
	all.symbols <- list(c("GOLD"), c("INTL"), c("TBIL3"), 
		c("GSPC"), c("GSPC", "TBIL3"), c("GSPC", "INTL", 
			"TBIL3"), c("GSPC", "INTL", "TBIL3", "GOLD"))
	for (lookback in c(8, 10, 12)) {
		for (delta in c(5, 10, 15, 20)) {
			...
			result$cagr <- 100 * (result$gain^(1/(result$periods/12)) - 
			  1)
			len <- length(result[, 1])
			r1 <- subset(result, symbol == strategy.symbol)$gain
			r2 <- subset(result, symbol == "GSPC")$gain
			tot <- length(r1)
			stat.string <- sprintf("%s > GSPC in %d/%d periods (%.2f%%)", 
			...
			fn <- sprintf("html/img/%d_year_window_%d_months.svg", 
			  delta, lookback)
			svg(fn, width = img.width, height = img.height)
			...
			if (delta == 20) 
			  y.max = 25
			fn <- sprintf("html/img/%d_year_window_%d_months_box_cagr.svg", 
			  delta, lookback)
			svg(fn, width = img.width, height = img.height)
			p <- ggplot(result, aes(x = symbol, y = cagr, 
			  fill = symbol)) + geom_boxplot()
			p <- p + xlab("") + ylab("CAGR [ % ]")
			p <- p + ggtitle(sprintf("1973-2014, %d year rolling gains, lookback=%d months", 
			  delta, lookback))
			p <- p + theme(axis.text.x = element_text(angle = 20, 
			  hjust = 1))
